...
class ArmEnv(object):
    ...
    def reset(self):
        ...
        '''set random position for hole'''
        self.hole_new_position = self.hole_init_position + (np.random.rand(3)-0.5) / 500
        self.hole_new_rotation = self.hole_init_rotation + (np.random.rand(4)-0.5) / 80
        self.hole_translation.setSFVec3f([self.hole_new_position[0], 2, self.hole_new_position[2]])
        self.hole_rotation.setSFRotation([self.hole_new_rotation[0], self.hole_new_rotation[1], self.hole_new_rotation[2], self.hole_new_rotation[3]])
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ainput = []
        ainput = [float(n) for n in input('action:').split()]
        print(ainput)
        state = env.test_action(ainput)
        print('state', state[:6])
        print('force', state[6:])
    env.restart()
    print("Hole init position 2", env.hole_init_position)
    print("Hole init rotation 2", env.hole_init_rotation)
    print('initial state 2', env.init_state)